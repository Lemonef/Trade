...
<script>
 ...
 function draw(s,lv){
  ...
  // CLEAN round gridlines (1/2/5 x 10^n nice step) instead of ugly evenly-spaced raw values ($24,342 etc.)
  const rs=rng/4,mag=Math.pow(10,Math.floor(Math.log10(rs))),nm=rs/mag,step=(nm<1.5?1:nm<3?2:nm<7?5:10)*mag;
  const lab=v=>v>=1000?"$"+(v/1000).toFixed(v%1000?1:0)+"k":"$"+Math.round(v);
  for(let v=Math.ceil(mn/step)*step; v<=mx; v+=step){const yy=Y(v);x.beginPath();x.moveTo(Pd,yy);x.lineTo(W-Pd,yy);x.stroke();x.fillText(lab(v),6,yy+4);}
 ...
 function render(){
  ...
  let rows=[];
  const LVC={'1x':'124,196,255','2x':'39,211,138','3x':'232,161,75'};
  const levTag=lv=>{if(L!=='all')return '';const c=LVC[lv];return ' <span style="padding:1px 8px;border-radius:999px;background:rgba('+c+',.15);color:rgb('+c+');border:1px solid rgba('+c+',.4);font-weight:700;font-size:9.5px">'+lv+'</span>';};
  if(P==='oos'){
